        ...
        while ($contador <= $diasEnMes) {
            ...
            for ($i = 1; $i <= 7; $i++) {
                ...
                if ($contador == intval(date('j')) && $mes_actual == intval(date('n')) && $anio_actual == intval(date('Y'))) {
                    ...
                    $calendario .= ' style="background-color: #72b5e1;">' . $contador . '</td>';
                    #$calendario .= ' style="background-color: #72b5e1;">' . '<a href="calendarioEventoVista.php?id_evento=15">' . $contador . '</a>' . '</td>';
                    #$calendario .= ' <?php onclick=calendarioEventoVista.php?id_evento=\'' . $this->miEventosEnFecha($contador, $mes_actual, $anio_actual)->getEvento() . '\'>';
