...
class Post{
    ...
    public static function buscaPost($id_post)
    {
        ...
        $query = sprintf("SELECT * FROM posts U WHERE U.id_post = '%s'", $conn->real_escape_string($id_post));
        $rs = $conn->query($query);
        $post=null;
        if ($rs) {
            ...
            $fila = $rs->fetch_assoc();
            if ($fila) {
                $post = new Post($fila['id_propietario'], $fila['titulo'], $fila['short_description'], $fila['descripcion'],$fila['imagen']);
